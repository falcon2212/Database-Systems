...
(select dlocation as location from dept_locations d  order by d.dlocation) union (select plocation as location from project p);

select d.essn, count(distinct d.dependent_name) from dependent d left outer join dependent w on d.essn = w.essn group by essn;

select d.essn, count(d.dependent_name) from dependent d FULL JOIN dependent w ON d.essn = w.essn group by d.essn;
